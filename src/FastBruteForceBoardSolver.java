...
public class FastBruteForceBoardSolver
{
	...
	{
		final int boardDimension = board.dimension, boardSize = board.dimension * board.dimension;
			...
			for(int tryValue = boardValueArray[y][x] + 1; tryValue <= boardDimension; tryValue++)
			{
				if(isValidValue(tryValue, x, y, boardValueArray, board))
				{
					...
					if(solvableSquares[i + 1] == null) /** End of board reached? */
					{
						 ...
						 */
						eventListener.onBoardSolutionComplete(board, boardValueArray.clone());
					}
					else
					{
						i++;
	...
	public void loadBoardData(Board board, int[][] values, int[][] solvableSquares)
	{
		int i = 0;
		...
		for(int y = 0; y < board.dimension; y++)
		{
			for(int x = 0; x < board.dimension; x++)
			{
				...
				values[y][x] = square.getValue();
								
				if(square instanceof DynamicSquare)
				{
					solvableSquares[i++] = new int[] {x, y};
				}
			}
		}
	}	
	
	final boolean isValidValue(int tryValue, int x, int y, int[][] boardValueArray, Board board)
	{
		...
		
		{
			...
			for(int j = b; j < b + board.boxHeight; j++)
			{
				for(int i = a; i < a + board.boxWidth; i++)
				{
